...
import { mailDataSource, options } from './data-source';
...
async function create() {
    mailDataSource.setOptions({
        ...options,
    });

    await mailDataSource.initialize();

    const bcryptService = new BcryptService();
    const hashedPassword = await bcryptService.hashPassword("123");

    const queryRunner = mailDataSource.createQueryRunner();
